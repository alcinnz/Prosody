/**
* This file is part of Prosody Templating Language (Copyright Adrian Cochrane 2017-2018, 2023).
*
...
namespace Prosody.Std {
    public Gee.Map<Bytes, Variable> parse_params(WordIter args) throws SyntaxError {
        var parameters = create_bytes_map<Variable>();
        var count = 0;
        foreach (var arg in args) {
            ...
            if (val == null) {
                val = key;
                key = new Bytes("$%i".printf(count++).data);
            }
    ...
    private class AutoescapeBuilder : TagBuilder, Object {
        public Gee.Map<Bytes,Gee.Map<uint8,string>> modes =
                create_bytes_map();

        public new void set(string key, Gee.Map<uint8, string> val) {
            modes[new Bytes(key.data)] = val;
        }
        public new Gee.Map<uint8, string> get(Bytes key) {return modes[key];}

        public Template? build(Parser parser, WordIter args) throws SyntaxError {
            var mode = args.next();
            args.assert_end();
            if (!modes.has_key(mode)) throw new SyntaxError.INVALID_ARGS(
                    @"Invalid {%% autoescape %%} mode: '$(s(mode))`");
    ...
    private class FilterBuilder : TagBuilder, Object {
        public Template? build(Parser parser, WordIter args) throws SyntaxError {
            ...
            args.assert_end();
            var variable = new Variable(new Bytes(@"_|$(s(filter_tail))".data));
        ...
        public Template? build(Parser parser, WordIter args) throws SyntaxError {
            ...
            if (s(sep) != "in")
                throw new SyntaxError.INVALID_ARGS("{%% for %%} expects the second to last " +
                        @"template argument to be 'in', got '$(s(sep))'");
    ...
    private class IfBuilder : TagBuilder, Object {
        public Template? build(Parser parser, WordIter args) throws SyntaxError {
            ...
            if (s(end_tag) != "endif" && s(end_tag) != "elif")
                throw new SyntaxError.UNBALANCED_TAGS(
                        "{%% if %%} must be closed with a {%% endif %%}");
            if (s(end_tag) == "endif") endtoken.assert_end();
    ...
    private class IncludeTag : Template {
        ...
        public override async void exec(Data.Data ctx, Writer output) {
            ...
            try {
                var source = yield absolute.load_bytes_async(null, null);
                var template = new Parser(source).parse();
                yield template.exec(ctx, output);
            } catch (Error err) {
    ...
    private class TemplateTagBuilder : TagBuilder, Object {
        public Template? build(Parser parser, WordIter args) throws SyntaxError {
            ...
            switch (s(variant)) {
            case "openblock": return new Echo(new Bytes("{%".data));
            case "closeblock": return new Echo(new Bytes("%}".data));
            case "openvariable": return new Echo(new Bytes("{{".data));
            case "closevariable": return new Echo(new Bytes("}}".data));
            case "openbrace": return new Echo(new Bytes("{".data));
            case "closebrace": return new Echo(new Bytes("}".data));
            case "opencomment": return new Echo(new Bytes("{#".data));
            case "closecomment": return new Echo(new Bytes("#}".data));
            default: throw new SyntaxError.INVALID_ARGS(
                    @"Expected '(open|close)(block|variable|brace|comment)' got '$(s(variant))'!");
            }
        }
    }
    ...
    private class MD5Filter : Filter {
        public override Data.Data filter0(Data.Data text) {
            var ret = Checksum.compute_for_bytes(ChecksumType.MD5, text.to_bytes());
            return new Data.Literal(ret);
        }
    }
