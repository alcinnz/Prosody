/**
* This file is part of Prosody Templating Language (Copyright Adrian Cochrane 2018, 2023).
*
...
namespace Prosody.xXML {
    public class XML : Data.Data {
        ...
        string[] locale;
        public XML(Xml.Node *node, string[] locale = xI18n.get_locales()) {
            this.node = node;
            this.locale = locale;
        }
        public XML.with_doc(Xml.Doc *node, string[] locale = xI18n.get_locales()) {
        ...
        public override Data.Data get(Bytes property) {
            ...
            for (Xml.Attr *iter = self->properties; iter != null; iter = iter->next) {
                if (iter->name == s(property)) return new XML(iter->children, locale);
            }
            // Second lookup amongst child nodes
            for (Xml.Node *iter = self->children; iter != null; iter = iter->next) {
                if (iter->name == s(property)) return new XML(iter, locale);
            }
        ...
        public override bool show(string defaultType, out Bytes text) {
            var self = get_localized();
            if (self == null) {text = EMPTY(); return true;}
            var type = self->get_prop("type");
            if (type == null) type = defaultType;

            text = new Bytes(self->get_content().data);
            switch (type) {
            case "xhtml":
                ...
                text = new Bytes(output.content().data);
                return is_sanitary(self);
            case "html":
                var html = Html.Doc.read_doc(s(text), "about:blank");
